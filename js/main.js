document.addEventListener('DOMContentLoaded', function() {
    var carrosselImages = document.querySelectorAll('.carrossel img');
    var currentIndex = 0;
    
    function showImage(index) {
      for (var i = 0; i < carrosselImages.length; i++) {
        carrosselImages[i].classList.remove('active');
      }
      carrosselImages[index].classList.add('active');
    }
    
    function nextImage() {
      currentIndex = (currentIndex + 1) % carrosselImages.length;
      showImage(currentIndex);
    }
    
    setInterval(nextImage, 3000); // Trocar a cada 3 segundos
  });

  function mostrar() {
    const enviar = document.querySelector("#enviar").value;
    alert(`Muito obrigado pela sujestão! ${enviar}`);
}
