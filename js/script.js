document.addEventListener('DOMContentLoaded', function() {
    var carrosselImagens = document.querySelectorAll('.carrossel img');
    var ver = 0;
    
    function showImage(index) {
      for (var i = 0; i < carrosselImagens.length; i++) {
        carrosselImagens[i].classList.remove('active');
      }
      carrosselImagens[index].classList.add('active');
    }
    
    function nextImage() {
      ver = (ver + 1) % carrosselImagens.length;
      showImage(ver);
    }
  ...
  document.getElementById("close-btn").addEventListener("click", function() {
    document.getElementById("popup").style.left = "-400px";
    setTimeout(function() {
